...
- [x] Step 12：实现 Discover/Library HTMX 页面：多 provider 聚合、dedupe badges、加入项目、下载/抓取/index 状态和错误恢复。
...
- [x] Step 16：实现 Deep Read UI：生成进度、九个固定栏目、证据抽屉、单字段重生成、人工编辑和版本历史。
...
- [x] Step 18：实现比较矩阵 UI/编辑/version；每个 cell 保存 evidence 或明确 `insufficient_evidence`。
  - API/UI：支持 project-scoped 选择 2–20 个来源、默认与自定义维度、持久 job 状态轮询、矩阵列表/详情、cell evidence 展开和 revision 历史；项目工作区增加比较入口。
...
- [x] Step 26：修复 SSE disconnect session race，接入 cancel event；实现 deadline-aware tool handlers，补齐 timeout/idempotency 合同。
  - SSE 断开：每回合持有 `_TurnScope`（独立取消事件 + worker-done + 一次性释放）；流提前退出只置位取消事件，排他锁等 worker 线程真正结束后才释放，并发回合无法同时修改同一 session；取消回合在阶段边界终止——普通回合消息恢复到上一持久化边界、run 转 `cancelled`，确认续跑中断开时保留协议闭合 transcript 不回滚已执行工具结果；断开后迟到事件在 emit 入口丢弃，终态以 Store run/session 记录为准。
...
- [x] Step 28：增加 deterministic product scenarios、live DeepSeek/manual provider smoke、质量 rubric、隐私/安全审查和数据备份恢复演练。
  ...
  - Live smoke 基础设施：新增 `scripts/smoke_live_deepseek.py`（真实模型跑 ≥2 张精读卡 + 三篇比较 + 综述 section，输出脱敏 usage/finish_reason/耗时 JSON；无 `PRA_LLM_API_KEY` 时 fail closed，已验证拒绝联网）与 `scripts/smoke_live_providers.py`（arXiv/S2/Crossref 手动实时检索，记录日期/query/结果数/错误码；key 只经环境变量读取）。命令、授权边界和记录模板见 `docs/evaluation.md`；真实 DeepSeek 执行状态与限制记录在本 Step 下，provider live smoke 尚未获得本轮授权且未执行。
  ...
  - 提交状态：Step 28 已由 `dc13026 test: add deterministic product journeys and evaluation tooling` 独立提交并推送；此前 Mimosa 对 `src/pragent/web/legacy/app.js` 浏览器端同源相对路径 `fetch()` 的 SSRF 误报不再构成提交阻塞，服务端 SSRF 防护仍由 `ingestion/safe_fetch.py` 与对应合同测试承担。
  - live 执行记录（2026-08-30，用户授权并配置 `deepseek-v4-flash` 后执行）：执行前修复真实模型暴露的问题——smoke 脚本两处缺陷（`Embedder` 缺参、index 计数结果不满足脱敏假设）；map 提示词未定义 `evidence_refs` 对象形状（prompt v2）；真实模型的字符串 refs 与同字段重复引用（schema 解析前确定性去重）；quote 空白/断词连字符/兼容字形漂移（忽略空白与连字符 + NFKC 的原文定位恢复，恢复结果仍为原文精确子串）与 quote 可定位性 verify（失败触发带原始证据载荷的一次 repair）；思考型模型空 content（显式 `max_tokens=16384`、空内容重试一次、错误带 finish_reason）。用户确认 `deepseek-chat` 已弃用，代码、示例与文档默认值统一为 `deepseek-v4-flash`；结构化精读/比较/综述调用接入官方 JSON Output，业务 schema/evidence/quote 校验和每卡一次 repair 上限均保持不变。后续脱敏诊断定位到 `limitations` 同时返回引用和 `insufficient_evidence=true`，现仅在存在引用时确定性把该布尔值纠正为 false，引用仍完整验证；两篇诊断复验中 Balanced 与 RecoAtlas 精读均成功（前者 0 repair、后者 1 repair），两篇比较成功，记录见 `docs/live-evidence/deepseek-smoke-20260830-diagnostic-2.json`。最终三篇全链复验按用户要求中断，用户改为人工测试；中断未生成 final JSON，不能宣称自动 live 全链通过。
- [x] Step 29：更新 README/架构/工作流/数据模型/安全/评估文档，构建 wheel，并完成从空目录安装到 `pra serve` 的发布验收。
  ...
  - 遗留边界：离线发布验收已完成；真实 DeepSeek 已产生成功单卡/两卡比较证据，但三篇比较与综述 section 的自动 live 全链由用户主动停止并改为人工测试，尚不能宣称 live 模型质量验收完成。默认模型固定为 `deepseek-v4-flash`，不再使用已弃用的 `deepseek-chat`；本会话不再发起 DeepSeek 或 provider live 请求。任何未完成 live 项都不得由 scripted 测试替代。
  - 当前收尾验证（2026-08-30）：Windows 完整回归 `419 passed, 1 skipped`，临时目录 95 MB；`pip check`、compileall、Phase 3/4 offline smoke、`security_review.py`（0 发现）、`backup_restore_drill.py`（通过，约 0.5s）、`node --check` 全部通过；当前 wheel SHA-256 为 `9adc328456a9109387aa39ad1c0df6d27ba3263092d68ad5b934bd3fb628a1c6`，资源清单、隔离安装、`pra 0.1.0` 与 Web runtime smoke 通过。`check_wheel.py` 新增 subprocess 超时、阶段输出和离线 pip 安装参数，避免成功安装后因版本检查网络等待而无限挂起。
