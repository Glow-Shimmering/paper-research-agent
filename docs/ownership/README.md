...
- 最新 Windows 完整离线测试：`354 passed, 1 skipped`；跳过项是当前普通账户没有创建符号链接权限时无法执行的导入安全测试。
...
详细复现步骤见 [第 1 周运行基线](week-01-baseline.md)，核心调用关系见 [核心链路图](core-flow.md)，会话概念边界见[会话记忆说明](session-memory.md)。继续开发时以[完整产品路线图](../plans/product-roadmap.md)的最早未完成 Step 为准。
