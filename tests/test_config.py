...
from paper_agent.config import _env_or_default, _find_env_file


def test_empty_environment_value_uses_default(monkeypatch):
    monkeypatch.setenv("PAPER_TEST_DEFAULT", "")

    assert _env_or_default("PAPER_TEST_DEFAULT", "fallback") == "fallback"


def test_find_env_file_prefers_explicit_relative_path(tmp_path):
    explicit = tmp_path / "custom.env"
    explicit.write_text("PAPER_LLM_MODEL=test\n", encoding="utf-8")
    cwd_env = tmp_path / ".env"
    cwd_env.write_text("PAPER_LLM_MODEL=cwd\n", encoding="utf-8")
...
def test_missing_explicit_env_does_not_fall_back(tmp_path):
    (tmp_path / ".env").write_text("PAPER_LLM_MODEL=cwd\n", encoding="utf-8")
...
def test_find_env_file_discovers_editable_repository_root(tmp_path):
    ...
    package = repo / "src" / "paper_agent"
    package.mkdir(parents=True)
    (repo / "pyproject.toml").write_text("[project]\nname='paper-agent'\n", encoding="utf-8")
    repo_env = repo / ".env"
    repo_env.write_text("PAPER_LLM_MODEL=repo\n", encoding="utf-8")
