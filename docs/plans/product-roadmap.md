...
- [x] Step 29：更新 README/架构/工作流/数据模型/安全/评估文档，构建 wheel，并完成从空目录安装到 `pra serve` 的发布验收。
  ...
  - 遗留边界：离线发布验收已完成；真实 DeepSeek smoke 已获授权并执行，但当前 `deepseek-v4-flash` 全链未通过（脱敏记录见 Step 28），不能宣称 live 模型质量验收完成。继续需要用户在“放宽每卡一次 repair 合同”与“改用非思考模型”之间作出新决策；Semantic Scholar/Crossref/arXiv provider live smoke 尚未获本轮授权且未执行。任何未完成 live 项都不得由 scripted 测试替代。
